...
#define P_512_SR(S4,S8,S9,S10,S11,S12,S13,S14,S15) \
  do { \
    __asm__ __volatile__ ( \
    	"ROR      %[S_4]  ,   #31     \n\t"\
		"ROR    %[S_11]  ,   #28      \n\t"\
...
#define P_512_SBOX2_ROR(S1,S2,S3,S4) \
  do { \
    __asm__ __volatile__ ( \
			"/*sbox first column  0,3,4,7           sbox1(s[0], ROR(s[3], 31),ROR(s[4], 28),  ROR(s[7], 25)); */         \n\t"      \
			"mvns    %[S_0],     %[S_0]            \n\t"      \
	...
	: /* output variables - including inputs that are changed */\
		[t1] "=r" (t1), [t2] "=r" (t2),\
		[S_0] "+r" (S1), [S_3] "+r" (S2), [S_4] "+r" (S3), [S_7] "+r" (S4) \
		: : );\
}while (0)
#define P_512_SBOX3_ROR(S1,S2,S3,S4) \
  do { \
	    __asm__ __volatile__ ( \
		"/*sbox first column  0,3,4,7           sbox1(s[0], s[3],ROR(s[4], 28),  ROR(s[7], 26)); */         \n\t"     \
		"mvns    %[S_0],     %[S_0]            \n\t"      \
		"ands    %[t1],    %[S_3], %[S_0]        \n\t"      \
		"eors    %[t1],    %[t1],    %[S_4] , ROR #28       \n\t"      \
...
#define P_512_SBOX1(S1,S2,S3,S4) \
  do { \
    __asm__ __volatile__ ( \
            "/*sbox   column*/         \n\t"\
   	        "mvns    %[S_0],     %[S_0]            \n\t"\
