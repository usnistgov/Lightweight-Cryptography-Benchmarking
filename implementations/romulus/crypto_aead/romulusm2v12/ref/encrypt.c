			 ...
			 )
{
  ...
  if (adlen == 0) { // AD is an empty string
    dual_lfsr(CNT);    
    //xlen = ad2msg_encryption (&m,CNT,s,k,t,n,108,xlen);      
  }
  else while (adlen > 0) {
      adlen = ad_encryption(&ad,s,k,adlen,CNT,104,n,t);
    }
  ...
  while (xlen > 0) {
    xlen = ad_encryption(&m,s,k,xlen,CNT,108,n,t);
  }
  ...
  *clen = mlen + n;
  ...
  if (mlen > 0) {
    nonce_encryption(N,CNT,s,k,t,n,100);  
    while (mlen > n) {
      mlen = msg_encryption(&m,&c,N,CNT,s,k,n,t,100,mlen);
    }
    rho(m, c, s, mlen, 16);
    c = c + mlen;
    m = m + mlen;    
...
)
{
  ...
  *mlen = clen;
  ...
  if (clen > 0) {    
    nonce_encryption(N,CNT,s,k,t,n,100);
    while (clen > n) {
      clen = msg_decryption(&m,&c,N,CNT,s,k,n,t,100,clen);
    }
    irho(m, c, s, clen, 16);
    c = c + clen;
    m = m + clen;
  ...
  if (adlen == 0) { // AD is an empty string
    dual_lfsr(CNT);    
    //xlen = ad2msg_encryption (&mauth,CNT,s,k,t,n,108,xlen);      
  }
  else while (adlen > 0) {
      adlen = ad_encryption(&ad,s,k,adlen,CNT,104,n,t);
    }
  ...
  while (xlen > 0) {
    xlen = ad_encryption(&mauth,s,k,xlen,CNT,108,n,t);
  }
  nonce_encryption(N,CNT,s,k,t,n,w);
