 ...
 * 
 * 
 * sESTATE_TweAES-128 ia a determinsitic AEAD based on the ESTATE mode
 * of operation and TweAES-128 tweakable block cipher. sESTATE uses the
 * 6-round variant of TweAES-128 in tag generation phase along with the
 * normal ESTATE mode of operation. It also uses tweak values 0x0f for
 * the tag generation phase, so that the tag generation and encryption
 * phase use tweaks with maximum distance codewords.
 * 
 ...
 * PT 	:
 * AD	: 
 * CT	: A6 4C F6 A7 37 71 F2 AA 5C 4D 75 45 E0 46 5B 08
 * 
 */
 ...
 * 
 **********************************************************************/
static void fcbc_star(u8 *tag, const u8 (*round_keys)[16], const u8 *in, const u64 inlen, const u64 in_blocks, const u8 *twks)
{
	u8 fifteen = 0x0f;
	u8 temp[CRYPTO_BLOCKBYTES];
	
	// process intermediate blocks with round-reduced
	// block cipher using tweak value fifteen
	for(u64 j=0; j<in_blocks-1; j++)
	{
		// compute T = F^15(K,T+I_j)
		memcpy(temp, tag, CRYPTO_BLOCKBYTES);
		xor_bytes(temp, &in[j*CRYPTO_BLOCKBYTES], CRYPTO_BLOCKBYTES);
		tweaes_enc(tag, &round_keys[0], &fifteen, CRYPTO_BC_REDUCED_NUM_ROUNDS, temp);
	}
	
	// process last block with distinct tweak
	// the tweak value depends on whether
	// the last block is partial or full
	if(PARTIAL_BLOCK_LEN(in_blocks,inlen) != CRYPTO_BLOCKBYTES)
	{
		...
		tweaes_enc(tag, &round_keys[0], &twks[1], CRYPTO_BC_NUM_ROUNDS, temp);
	}
	else
	{
		...
		tweaes_enc(tag, &round_keys[0], &twks[0], CRYPTO_BC_NUM_ROUNDS, temp);
	}
}
 ...
 * 
 **********************************************************************/
static void mac(u8 *tag, const u8 (*round_keys)[16], const u8 *nonce, const u8 *ad, const u64 adlen, const u64 ad_blocks, const u8 *pt, const u64 ptlen, const u64 pt_blocks)
{
	u8 twks[2] = { 0 };
	...
	if(adlen == 0 && ptlen == 0)
	{
		...
		twks[0] = 0x08;
		tweaes_enc(&tag[0], &round_keys[0], &twks[0], CRYPTO_BC_NUM_ROUNDS, &temp[0]);
	}

	// generate tag when ad and/or pt are non-empty
	twks[0] = 0x0f;
	tweaes_enc(&tag[0], &round_keys[0], &twks[0], CRYPTO_BC_REDUCED_NUM_ROUNDS, &temp[0]);
	
	if(adlen > 0)
	{
		// process non-empty ad blocks
 ...
 * 
 **********************************************************************/
static void ofb(u8 *out, u64 *outlen, const u8 (*round_keys)[16], const u8 *iv, const u8 *in, const u64 inlen, const u64 in_blocks)
{
	u8 twk = 0x00;
	...
	*outlen = 0;
	...
	for(u8 i=0; i<in_blocks-1; i++)
	{
		...
		tweaes_enc(iv_, &round_keys[0], &twk, CRYPTO_BC_NUM_ROUNDS, temp);
...
)
{
	...
	u8 round_keys[CRYPTO_BC_NUM_ROUNDS+1][16];
	_TWEAES_ENC_ROUND_KEY_GEN(&round_keys[0], &k[0]);
