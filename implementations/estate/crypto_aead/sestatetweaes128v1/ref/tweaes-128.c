 ...
 * [2,3]), in that it accepts a very short tweak of size 1 nibble. We
 * also implemented a round-reduced version of TweAES-128 called
 * TweAES-128-6.
 * 
...
void shift_rows_inv(u8 *state_bytes)
{
    ...
    // zeroth row is not shifted at all
    
    // first row shifts by 1
 ...
 * @name	:	tweaes_enc
 * 
 * @note	:	TweAES-128 and TweAES-128-6 encryption function.
 * 
 **********************************************************************/
void tweaes_enc(u8 *ct, const u8 (*round_keys)[16], const u8 *twk, const u8 rounds, const u8 *pt)
{
    ...
    sub_bytes(&ct[0]);
    shift_rows(&ct[0]);
    if(rounds == CRYPTO_BC_REDUCED_NUM_ROUNDS)
    {
 ...
 * @name	:	tweaes_dec
 * 
 * @note	:	TweAES-128 and TweAES-128-6 decryption function.
 * 
 **********************************************************************/
void tweaes_dec(u8 *pt, const u8 (*round_keys)[16], const u8 *twk, const u8 rounds, const u8 *ct)
{
    ...
    if(rounds == CRYPTO_BC_REDUCED_NUM_ROUNDS)
    {
		// apply mixcolumns inverse operation
		// for TweAES-6
